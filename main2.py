...
class StarkHUDWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self.yolo_model = YOLO("yolo11m.pt")  # Load YOLO model
    ...
    def update(self, dt):
        ...
        if frame is not None:
            ...
            results = self.yolo_model.predict(source=frame, show=False, conf=0.5)

            # Draw bounding boxes and labels on the frame
            for result in results[0].boxes:
                x1, y1, x2, y2 = map(int, result.xyxy[0])  # Get bounding box coordinates
                label = int(result.cls.item())  # Convert class label to int
                confidence = float(result.conf.item())  # Convert confidence to float

                # Draw green rectangle for detected objects
                ...
                text_x = x1
                text_y = y1 - 5 if y1 - 5 > 10 else y1 + 15  # Adjust position if too close to the top

                # Draw background rectangle for text
                cv2.rectangle(frame, (text_x, text_y - text_size[1] - 2), 
            ...
            # Update texture with the annotated frame
            buf = frame.tobytes()
